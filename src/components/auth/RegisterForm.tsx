...
export function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!data.success) { setError(data.error); setIsLoading(false); return; }
    ...
    <div className="w-full max-w-sm mx-auto px-4">
      ...
      <div className="relative my-5">
        ...
        <div className="relative flex justify-center">
          <span className="px-3 text-xs" style={{ background: "var(--color-background)", color: "var(--color-text-muted)" }}>
            or
          </span>
        </div>
      </div>
      ...
      {error && (
        <div className="rounded-lg px-3 py-2.5 text-sm mb-4" style={{ background: "var(--color-danger-muted)", color: "var(--color-danger)" }}>
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="password" type="password" placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} disabled={isLoading} />
        </div>
